...
def stealth_scan(target_ip, port_range):
    ...
    for i in ports_to_scan: ## +1 due to range function
        ...
        try:
            SYNACKpkt = sr1(IP(dst = target_ip)/TCP(sport = srcport, dport = i, flags = "S"), timeout = 2)
        ...
        ## RST packet
        RSTpkt = IP(dst = target_ip)/TCP(sport = srcport, dport = i, flags = "R")
    ...
    print("List Output from scan, for future DB use")
    print(open_list)
    
    database_write(open_list[0], str(set(open_list[1:])), "TEST") #[1:] means to end of list
    print(open_list[0], str(set(open_list[1:])), "TEST")

def database_write(IP, PORT, SCANTYPE):
    try:
        sqliteConnection = sqlite3.connect('logec_db')
        cursor = sqliteConnection.cursor()
        print("Successfully Connected to SQLite")

        sqlite_insert_query = f"""INSERT INTO PortScan
                            (IP, PORT, ScanType) 
                            VALUES 
                            ({IP},{PORT},{SCANTYPE})"""
